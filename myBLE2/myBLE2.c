...
//
// this is alpha, but the scooter drives fine and all the controls work normally
...
static byte initStr[] = {0x5a, 0xa5, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    ...
    if (rxBytes > 0) {
      ...
      for (; (idx < rs) && doMore; idx++) {
        if ((ifRX[ifb][idx] == 0x5a) && doMore) {
          if ((ifRX[ifb][idx + 1] == 0xa5 && doMore)) {
            ...
            if ((cksum & 0xff) != ifRX[ifb][j]
              ...
              if (ECU) {
                ...
                // these flags
                int ECU_read = (2 << 30);   // had read
                int ECU_write = (2 << 29);  // had write
                int ECU_grpChange = (2 << 28);  // did update
                int ECU_valChange = (2 << 27);  // did change
                int ECU_error = (2 << 26);  // had error
                int ECU_bglow = (2 << 25);  // low priority bg reading
                int ECU_noRemap = (2 << 23);// abort processing

                //                ECU32[0x9fC] = ECU_noRemap;
                //                ECU32[0x7f8] = ECU_noRemap;
                ...
                if (dECU) {
                  ...
                  //                  if ( (*ECUptr && ECU_noRemap)>0 )
                  //      long test=(*ECUptr & ECU_noRemap);
                ...
                */
                ...
                if (doXmit) {
                  ...
                  // just "test routing" for now
                  if ((ifb == 0) || (wr_ack)) {   // hardcode write ack
          ...
          //BUG - when bad characters are present, this copies them; they should be dropped.
